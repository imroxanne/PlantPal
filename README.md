...
- Telegram-напоминания о поливе (Vercel Cron + Bot API, раз в день)
- Настройки уведомлений (включение, выбор времени)

> **Vercel Hobby plan:** cron запускается раз в день (ограничение Hobby).
> Точное время `notification_time` пользователя сохраняется, но не гарантируется —
> напоминание приходит при ежедневном запуске cron.
